...
+ (NSString *)getFormatTime:(long long)time {
    if([NSString stringWithFormat:@"%lld",time].length>=13){
        time = time/1000;
    }
    NSDate *date = [NSDate dateWithTimeIntervalSince1970:time];
    NSDateFormatter  *formatter = [[NSDateFormatter alloc] init];
    
    NSUInteger calendarFlag = NSYearCalendarUnit | NSMonthCalendarUnit | NSDayCalendarUnit;
    NSDateComponents *component1 = [[NSCalendar currentCalendar] components:calendarFlag fromDate:date];
    NSDateComponents *component2 = [[NSCalendar currentCalendar] components:calendarFlag fromDate:[NSDate date]];
